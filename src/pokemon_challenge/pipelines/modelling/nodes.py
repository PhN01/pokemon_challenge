...
class torch_regression_model:
    ...
    def init_model(self):
        ...
        assert not any(
            [
                param is None
                for param in [self.num_epochs, self.early_stopping, self.input_dim]
            ]
        )
        ...
        for i in range(1, len(self.dnn_arch)):
            model.add_module(
                "dense_{}".format(i), nn.Linear(self.dnn_arch[i - 1], self.dnn_arch[i])
            )
        ...
        self.optimizer = AdamW(
            params_to_update, lr=self.learning_rate, weight_decay=self.weight_decay
        )
    ...
    def train(self, data_loaders_dict):
        ...
        ), "Model not initialized. Call init_model() or load_model() before running train()."
        ...
        for epoch in range(self.num_epochs):
            ...
            if (
                ...
                and early_stopping_counter > 0
            ):
                log.info(
                    f"Stopped training because of no improvement of the validation score for "
            ...
            for phase in ["train", "val"]:
                ...
                log.info(
                    f"{phase} {loss.__class__.__name__} loss: {epoch_loss:.6f} root of loss: {epoch_roloss:.6f}"
                )
                ...
                if phase == "val":
                    ...
                    if epoch_loss < best_loss:
                        ...
                        torch.save(
                            best_model_wts, f"{self.output_dir}/best_model_weights.pth"
                        )
                        early_stopping_counter = 0
                    else:
        ...
        if "test" in data_loaders_dict.keys():
            ...
            log.info(
                f"test {loss.__class__.__name__} loss: {epoch_loss:.6f} root of loss: {epoch_roloss:.6f}"
            )
            log.info("-" * 70)
            log.info("-" * 70)
    ...
    def load_model(self):
        ...
        if torch.cuda.is_available():
            ...
            self.model = torch.load(
                f"{self.output_dir}/best_model.pth", map_location=torch.device("cpu")
            )


def get_predictions_and_true_labels(data_loader, fitted_model):
...
def get_predictions(data_loader, fitted_model):
    r""" Function to get the true labels and the predictions by a fitted model for a provided dataset.
...
def eval_ensemble_for_dataset(model_dict, dataloader):
    ...
    for model_path, model in tqdm(model_dict.items(), desc="Models"):
        ...
        dataset_summary[model_name]["mse"] = float(
...
def fit_dnn_model(
    ...
    if torch.cuda.is_available():
        ...
        train_tensors = data_utils.TensorDataset(
            torch.FloatTensor(np.array(X_train)), torch.FloatTensor(np.array(y_train))
        )

        val_tensors = data_utils.TensorDataset(
            torch.FloatTensor(np.array(X_val)), torch.FloatTensor(np.array(y_val))
        )

        test_tensors = data_utils.TensorDataset(
            torch.FloatTensor(np.array(X_test)), torch.FloatTensor(np.array(y_test))
        )

    train_loader = data_utils.DataLoader(train_tensors, batch_size=1024, shuffle=True)

    val_loader = data_utils.DataLoader(val_tensors, batch_size=1024, shuffle=False)

    test_loader = data_utils.DataLoader(test_tensors, batch_size=1024, shuffle=False)
...
def eval_dnn_models(
    ...
    train_loader = data_utils.DataLoader(train_tensors, batch_size=1024, shuffle=False)
    ...
    for dataset, dataloader in tqdm(data_loaders_dict.items(), desc="Datasets"):
        ...
        dataset_summary, all_preds, true_labels = eval_ensemble_for_dataset(
            model_dict, dataloader
        )
        ...
        model_summaries_dict[dataset]["ensemble"]["mse"] = float(
            mean_squared_error(true_labels, avg_preds)
        )
        ...
        if dataset == "train":
            global lm
            log.info(
                "Training linear regression model on individual model predictions."
            )
        ...
        model_summaries_dict[dataset]["weighted_ensemble"][
            "preds"
        ] = weighted_avg_preds.tolist()
        model_summaries_dict[dataset]["weighted_ensemble"]["mse"] = float(
            mean_squared_error(true_labels, weighted_avg_preds)
        )
    ...
    for dataset in data_loaders_dict.keys():
        ...
        for model_path in model_dict.keys():
            model_name = model_path.split("/")[-1]
            log.info(
                f"Model {model_name}.  MSE: {model_summaries_dict[dataset][model_name]['mse']}"
            )
        log.info(f"Ensemble.  MSE: {model_summaries_dict[dataset]['ensemble']['mse']}")
        log.info(
            f"Weighted Ensemble.  MSE: {model_summaries_dict[dataset]['weighted_ensemble']['mse']}"
        )


def predict_available_pokemon_performance(
    available_pokemon: pd.DataFrame,
    battles: pd.DataFrame,  # train_battles: pd.DataFrame,
    dnn_arch_1: dict,
    dnn_arch_2: dict,
    dnn_arch_3: dict,
    dnn_arch_4: dict,
    ...
    if torch.cuda.is_available():
        data_tensor = data_utils.TensorDataset(
            torch.cuda.FloatTensor(np.array(battles))
        )

    else:
        data_tensor = data_utils.TensorDataset(torch.FloatTensor(np.array(battles)))

    dataloader = data_utils.DataLoader(data_tensor, batch_size=1024, shuffle=False)
    ...
    log.info(
        "Creating weighted average over predictions using ensemble regression model."
    )
    ...
    for i, pokemon in enumerate(enemy_pokemons):
        ...
        available_pokemon[f"hppr_{pokemon}".lower()] = np.minimum(
            np.maximum(available_pokemon[f"res_{pokemon}".lower()], 0)
            / available_pokemon["HP_1"],
            1,
        )
