...
def lp_optimization(
        available_performance: pd.DataFrame, submission_file: pd.DataFrame
):
    log = logging.getLogger(__name__)

    # prepare array with optimization metric (HPPR)
    battle_outcomes = available_performance[
        ['hppr_caterpie', 'hppr_golem', 'hppr_krabby', 'hppr_mewtwo', 'hppr_raichu', 'hppr_venusaur']]
    ...
    for n in range(int(num_available_pokemon/10)):
        solver.Add(solver.Sum([x[i, j] for j in range(num_enemy_pokemon) for i in range(n*10,(n+1)*10)]) <= 1)
    ...
    solver.Add(solver.Sum(
        [x[i, j] * prices[i] for i in range(num_available_pokemon) for j in
         range(num_enemy_pokemon)]) <= max_budget)
    ...
    for j in range(num_enemy_pokemon):
        for i in range(num_available_pokemon):
            if x[i, j].solution_value() > 0:
                submission_file.loc[j, 'SelectedPokemonID'] = available_performance.SelectedPokemonID[i]
                log.info(f'Pokemon {available_performance.Name_1[i]} (Level {available_performance.Level_1[i]}) assigned '
...
class GeneticPokemons(object):
    ...
    def __init__(self, data, num_lineups, duration=60, opt_metric='result', p_mutation=0.2):
        self.data = data
    ...
    def mate_lineups(self, lineup1, lineup2):
        ...
        while True:
            ...
            def mutation(pokemon):
                mutate = np.random.choice([0, 1], p=[1-self.p_mutation,self.p_mutation])
                if not mutate:
                    return pokemon
                else:
    ...
    def check_valid(self, lineup):
        ...
        cost_list = []
        names_list = []
        for i, opponent in enumerate(['caterpie','golem','krabby','mewtwo','raichu','venusaur']):
